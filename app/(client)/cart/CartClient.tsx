...
export default function CartClientpage(): React.JSX.Element {
      ...
      {isSignedIn ? (
        <Container>
          {cartProducts?.length ? (
            <>
              {/* Header */}
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 py-4 md:py-5">
              ...
              <div className="grid lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
                  ...
                  <div className="border bg-white rounded-lg shadow-sm">
                    {cartProducts?.map(({ product }) => {
                      ...
                      return (
                        <div
                          key={product?._id}
                          className="border-b p-3 md:p-4 last:border-b-0 flex items-center justify-between gap-3 md:gap-4 hover:bg-gray-50 transition-colors"
                        >
                          <div className="flex flex-1 items-start md:items-center gap-3 md:gap-4 min-h-24 md:min-h-32">
                            {product?.images && (
                              <Link
                              ...
                              >
                                <Image
                                  ...
                                  width={80}
                                  height={80}
                                  className="w-16 h-16 md:w-24 md:h-24 lg:w-32 lg:h-32 object-cover group-hover:scale-105 transition-transform duration-200"
                                />
                              </Link>
                            )}
                            <div className="flex flex-1 flex-col justify-between h-full py-1 min-w-0">
                              <div className="space-y-1 md:space-y-2">
                                <Link
                                  href={`/product/${product?.slug?.current}`}
                                  className="font-semibold text-sm md:text-base line-clamp-2 hover:text-blue-600 transition-colors break-words"
                                >
                                  {product?.name}
                                </Link>
                              ...
                              <div className="text-gray-500 flex items-center gap-3 mt-2">
                                <TooltipProvider>
                                  <Tooltip>
                            ...
                            <div className="flex flex-col items-end justify-between h-full py-1 space-y-2">
                              <PriceFormatter
                                amount={(product?.price as number) * itemCount}
                                className="font-bold text-sm md:text-base lg:text-lg text-gray-900"
                              />
                ...
                {/* Order Summary & Checkout Section - Full width on mobile, 1/3 on desktop */}
                <div className="lg:col-span-1">
                  <div className="sticky top-4 space-y-4 md:space-y-6">
                    <div className="w-full bg-white p-4 md:p-6 rounded-lg border shadow-sm">
              ...
              {/* Mobile Checkout Button - Only show when checkout form is NOT visible */}
              {!showCheckoutForm && (
                <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t p-4 shadow-lg z-40">
                  <div className="flex justify-between items-center">
                    ...
                    <Button
                      className="bg-green-600 hover:bg-green-700 font-semibold text-white px-6"
